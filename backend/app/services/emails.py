...
def verification_email(user: User) -> EmailMessage:
    ...
    return _message(
        ...
        f"Ссылка действует двое суток. Если ты не регистрировался в Self Forge, "
        f"просто удали это письмо.\n",
    )
...
def password_reset_email(user: User) -> EmailMessage:
    ...
    return _message(
        ...
        f"Ссылка действует час и срабатывает один раз. Если ты не просил сброс, "
