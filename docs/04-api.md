...
тренировочного дня по порядку, длина равна `days_per_week`. Без него все дни в основном месте.
Дни порядковые, к дням недели не привязаны. Оба эндпоинта требуют завершённого онбординга
(иначе 409). `POST /programs` переводит прежнюю активную программу в `abandoned`.
